...
import { initCommandRoute, submitCommand, getCommandStatusRoute } from "./routes/commands";
...
import { getMetrics, getTracer, setMetrics, setTracer, InMemoryMetrics, InMemoryTracer } from "@erp/observability";
import { CommandHandlerBase, type CommandExecutionContext } from "./commands/CommandHandlerBase";
...
> {
  ...
  async executeBusinessLogic(
    envelope: CommandEnvelope<{ rowId: string; columnId: string; value: string }>,
    context: CommandExecutionContext
  ) {
    const { rowId, columnId, value } = envelope.payload;
    const tenant = envelope.tenantId;
    const workbook = envelope.workbookId || "pilot-v1-small";
    ...
    try {
      await context.ledger.createTransfer({
        ...
        commandLineIndex: 0,
        ledgerGroupId: crypto.randomUUID?.() || "00000000-0000-0000-0000-000000000000",
        transferCode: 1,
        mode: "single_phase",
        status: "posted",
      });
...
export async function startApi(): Promise<void> {
  ...
  const poller = new OutboxPoller(outboxRepo, metrics, { maxEventsPerPoll: 1000, maxBytesPerPoll: 2 * 1024 * 1024 });
  const pollerLoop = new OutboxPollerLoop(poller, connectionManager, 750);
  pollerLoop.start();

  // Rate limiter (ordinary edits)
  const rateLimiter = new RateLimiter({ maxTokens: 120, refillRate: 2, refillIntervalMs: 1000 });

  // Native HTTP server (no extra runtime deps beyond node)
  const server = http.createServer(async (req, res) => {
    // CORS for local dev web
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader("Access-Control-Allow-Headers", "content-type,x-tenant-id,x-user-id,x-trace-id,x-correlation-id,last-event-id");
    ...
    try {
      ...
      if (url.pathname === "/api/commands" && req.method === "POST") {
        ...
        const userId = req.headers["x-user-id"]?.toString() || "00000000-0000-0000-0000-000000000099";
        const traceparent = req.headers["x-trace-id"]?.toString() || req.headers["traceparent"]?.toString();
        ...
        const rl = rateLimiter.tryConsume(tenantId, body.commandType || "cell.update", "ordinary" as any);
        if (!rl.allowed) {
        ...
        const resp = await submitCommand(tenantId, userId, body, traceparent, correlationId);
        res.writeHead(resp.status === "committed" ? 200 : 202, { "content-type": "application/json" });
      ...
      if (url.pathname === "/api/workbooks" && req.method === "GET") {
        ...
        const workbookId = url.searchParams.get("workbookId") || "pilot-v1-small";
        ...
        res.end(
          JSON.stringify({
            tenantId,
            workbookId,
            rows: [...rowMap.entries()].map(([rowId, values]) => ({ rowId, values })),
          })
        );
        return;
      }
