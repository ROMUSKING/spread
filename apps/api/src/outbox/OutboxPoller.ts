...
export class OutboxPoller {
  ...
  async pollOnce(
    ...
    if (tenantIds.length === 0) {
      return { nextHighWatermark: highWatermark, events: [], syncRequired: false };
    }
    ...
    if (
      ...
      const decisionSpan = tracer.startSpan("erp.outbox.full_refresh_decision", {
    ...
    try {
      ...
      pollSqlSpan.setAttribute("erp.rows_returned", envelopes ? envelopes.length : 0);
      pollSqlSpan.setAttribute("erp.duration_ms", sqlDurationMs);
      pollSqlSpan.setAttribute("rows_returned", envelopes ? envelopes.length : 0);
      pollSqlSpan.setAttribute("duration_ms", sqlDurationMs);
      pollSqlSpan.end();
    }

    this.metrics.observe("erp_outbox_envelope_rows_scanned_total", envelopes.length);
    ...
    for (const env of envelopes) {
      ...
      const subscribedWorkbooks = subscriptions.workbookIdsByTenant.get(env.tenantId);
      if (env.workbookId && (!subscribedWorkbooks || !subscribedWorkbooks.has(env.workbookId))) {
        skippedNoLocalSubscriber++;
        continue;
      }

      // Byte budget enforcement
      if (accumulatedBytes + env.payloadSizeBytes > this.options.maxBytesPerPoll) {
        budgetBreached = true;
        // Rollback nextWatermark to the previous item's outboxId so we resume from here next time
        const index = envelopes.indexOf(env);
        if (index > 0) {
    ...
    demandSpan.setAttribute("events_skipped_no_local_subscriber", skippedNoLocalSubscriber);
    ...
    if (budgetBreached) {
      ...
      const decisionSpan = tracer.startSpan("erp.outbox.full_refresh_decision", {
        ...
        workbook_id_hash: envelopes[deliverableMetadata.length]?.workbookId ? hashId(envelopes[deliverableMetadata.length]!.workbookId) : "",
    ...
    let payloadRows: Array<{ outboxId: string; payload: unknown; payloadHash: string; payloadRef?: string }> = [];
    ...
    if (deliverableIds.length > 0) {
      ...
      try {
        ...
        this.metrics.observe("erp_outbox_poll_sql_seconds", payloadDuration / 1000);
        this.metrics.observe("erp_outbox_payload_bytes_fetched_total", accumulatedBytes);
    ...
    for (const meta of deliverableMetadata) {
      ...
      if (!payloadRow) {
        throw new Error(`Data integrity breach: Payload missing for outbox_id ${meta.outboxId}`);
      }
    ...
    if (fullEvents.length > 0) {
      this.metrics.observe("erp_outbox_payload_ref_ratio", payloadRefsCount / fullEvents.length);
    }
