...
export abstract class CommandHandlerBase<TPayload, TResponse> {
  ...
  async execute(envelope: CommandEnvelope<TPayload>, context: CommandExecutionContext): Promise<CommandOutcome<TResponse>> {
    ...
    try {
      ...
      context.metrics.increment("command_committed_total", { command_type: envelope.commandType });
      return { status: "committed", response: result };
    } catch (error) {
      span.recordException(error);
      context.metrics.increment("command_failed_total", { command_type: envelope.commandType });
      return { status: "failed", code: "COMMAND_EXECUTION_FAILED", message: error instanceof Error ? error.message : "Unknown command failure" };
    } finally {
      span.end();
    }
  }
  ...
  protected assertEnvelope(envelope: CommandEnvelope<TPayload>): void {
    if (!envelope.commandId || !envelope.requestHash || !envelope.tenantId) {
      throw new Error("Invalid command envelope: commandId, requestHash, and tenantId are required.");
    }
  }

  protected abstract executeBusinessLogic(envelope: CommandEnvelope<TPayload>, context: CommandExecutionContext): Promise<TResponse>;
}
