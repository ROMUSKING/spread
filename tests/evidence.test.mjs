...
test('fulfillment.allocate reserves stock for a confirmed order line', async () => {
  const {
  ...
  const confirmResult = await processor.processCommand('t1', 'u1', {
    commandId: 'cmd-so-confirm-2',
    commandType: 'salesOrder.confirm',
    payload: { orderId: 'SO-002' },
    workbookId: '00000000-0000-0000-0000-000000000015'
  }, null, null, '00000000-0000-0000-0000-000000000015');
  assert.equal(confirmResult.status, 'committed');

  const allocateResult = await processor.processCommand('t1', 'u1', {
    commandId: 'cmd-so-alloc-2',
  ...
  assert.equal(allocateResult.status, 'committed');
  assert.equal(cellsMap.get('t1:00000000-0000-0000-0000-000000000015:SO-002-L1:status'), 'ALLOCATED');
  assert.equal(cellsMap.get('t1:00000000-0000-0000-0000-000000000015:SO-002-HDR:status'), 'ALLOCATED');
  assert.equal(cellsMap.get('t1:00000000-0000-0000-0000-000000000014:PROD-001:w1:quantity_reserved'), '3');
