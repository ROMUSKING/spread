# Spreadsheet-Native ERP Technical Specification v0.15.0

**Date:** 2026-06-26  
**Status:** Phase 0 implementation-readiness baseline with AI coding-agent execution roadmap
...
**Version note:** This is **v0.15.0**, not v1.0. Version 1.0 remains reserved for a release-candidate baseline after Phase 0 evidence exists.  
...
Version 0.15.0 is the active Phase 0 implementation-readiness and AI coding-agent execution baseline. The core product thesis remains unchanged: build a TypeScript-first, PostgreSQL-backed, spreadsheet-native ERP where every visible cell is a permissioned, validated, auditable projection of normalized business data.


The v0.15.0 line adds a detailed implementation roadmap for AI coding agents. It converts the validated architecture pack into an execution plan with agent operating rules, work-order dependencies, allowed file scopes, acceptance criteria, stop conditions, validation commands, and PR handoff requirements. This is an execution-layer change, not an expansion of MVP product scope.

The v0.15.0 roadmap makes one rule explicit: AI coding agents may accelerate implementation, but they may not weaken authority boundaries. Every mutation remains command-mediated; every outbound event remains outbox-derived; every derived-plane result remains revalidated; and every post-MVP plane remains evidence-gated until its P1 gate passes.

The v0.14 line adds a governed external-systems integration strategy: inbound external data is staged, scanned, schema-validated, classified, mapped, and converted into command proposals before any ERP mutation; outbound external data is delivered from durable outbox events only. No connector, marketplace adapter, third-party API, broker, CDC bridge, TigerBeetle adapter, pgvector retrieval path, DuckDB snapshot, or UI tile may bypass command handlers or become an implicit authority for operational state.

The v0.15.0 final-promotion review closure completes active-version cleanup, makes the integration staging-to-command handoff explicit in the main implementation narrative, deepens credential/KMS envelope-encryption guidance, adds integration observability examples and a revoked-credential/schema-mismatch negative fixture, and keeps UI tiling explicitly post-vertical-slice.

The earlier v0.13 refinements remain active: TigerBeetle is the post-MVP target numeric ledger plane for conserved numeric movement; pgvector is the post-MVP semantic retrieval plane over governed derived chunks; DuckDB is the post-MVP analytical execution plane over derived snapshots; and all cross-plane results require revalidation before user-visible display or command proposal.

The original v0.12 procedural change remains: Phase 0 begins with evidence-producing workstreams that prove the system can safely mutate data, recover from ambiguous outcomes, stream updates without harming commit latency, enforce invariants through CI, and validate transactional-batch partitions. `LISTEN/NOTIFY` remains optional wake-up only; durable outbox polling with jitter and local-subscriber filtering is the default until evidence proves otherwise.

**Integration Security Posture:** All inbound integration payloads must pass authentication, rate limiting, content-type allow-listing, payload-size checks, payload hashing, malware scanning or quarantine, schema validation, data classification, redaction, service-account scope checks, and external mapping lookup before command proposal. Credentials are stored only as `credential_ref` values backed by KMS/envelope encryption and audited secret-access paths. The threat model summary is `docs/security/threat-model-summary.md`.

**v0.15.0 Final Promotion Closure:** Active documentation now uses v0.15.0 as the implementation baseline. The exact closure changes are: (1) stale active-version references and validation expectations are removed or classified as archival; (2) `docs/dev/command-lifecycle.md` contains one concise integration staging -> validation gates -> command proposal -> command-handler sequence; (3) `docs/security/integration-credential-management.md` defines a tenant-scoped DEK + KMS master-key envelope-encryption profile and in-flight revocation propagation; (4) `docs/observability/phase0-observability.md` includes integration staging, scan, dead-letter, and credential-revocation metrics; (5) negative fixtures now include a combined revoked-credential + schema-mismatch case; and (6) `P1-UX-001` remains explicitly post-vertical-slice.



**v0.15.0 AI Coding-Agent Execution Baseline:** The implementation sequence is now decomposed into work orders that can be executed by coding agents under repository guardrails. Required artifacts are `AGENTS.md`, `docs/implementation/ai-coding-agent-roadmap.md`, `docs/implementation/phase0-agent-work-orders.md`, `docs/implementation/agent-operating-model.md`, `docs/implementation/agent-pr-validation-playbook.md`, and `docs/gates/P0-EXEC-001-ai-agent-implementation-readiness.md`. The roadmap is subordinate to the existing P0 gates and cannot reorder them.

**Vertical Slice Requirement (mandatory v0.15.0):** Single safe cell edit must complete end-to-end (command -> PostgreSQL transaction -> outbox polling -> SSE -> unknown-outcome recovery) before UI tiling, external connector runtime, TigerBeetle cutover, pgvector retrieval, DuckDB analytics, CDC fan-out, or broad editable-cell work.

The Phase 0 sequence is locked:
...
| Area | v0.11 position | v0.14 position |
|---|---|---|
...
### 2.12 Normative refinements from v0.14.1 to v0.14.2

| Area | v0.14.1 residual risk | v0.14.2 refinement |
|---|---|---|
| Main spec synchronization | v0.14.1 closure material was represented in supporting docs, but the active spec narrative still contained stale v0.13/v0.14 language. | Active spec promoted to v0.14.2 and summarizes integration security, UI preparedness, credential management, threat model, and validation cleanup. |
| Integration handoff | Inbound staging controls existed but the staging -> command proposal -> command handler handoff was too implicit. | `docs/dev/command-lifecycle.md` now includes an integration handoff sequence with validation gates before command creation. |
| Credential management | `credential_ref` and service-account scope existed, but KMS/envelope encryption and secret-scanning details were light. | `docs/security/integration-credential-management.md` defines KMS envelope encryption, rotation overlap, revocation propagation, and CI secret-scanning checks. |
| UI scope | Tiled workspace docs existed but needed stronger protection against premature MVP work. | P1-UX-001 is explicitly post-vertical-slice; Phase 0 only preserves metadata hooks and command-safe transposed view contracts. |
| Integration evidence | Positive integration fixture existed, but no negative/quarantine payload was packaged. | Add negative integration fixture and manifest references for quarantine/dead-letter test paths. |
| Manifest hygiene | Required docs contained duplicates and older active references. | Required docs deduplicated and validation now rejects duplicate requiredDocs and stale active spec/gate references. |
...
## v0.14.1 Review Closure: External Integration Security, UI Preparedness, and Packaging Cleanup

v0.14.1 closes the external-integration readiness review without widening MVP scope.

### Normative additions

1. Inbound integration security is release-blocking: payloads must be authenticated, rate-limited, size-limited, content-type checked, malware-scanned or quarantined, schema-validated, classified, and redacted before command proposal.
2. Integration credentials use scoped service accounts and secret references; plaintext credentials are prohibited in PostgreSQL metadata, payloads, logs, outbox events, pgvector chunks, DuckDB snapshots, and support bundles.
3. Command transaction boundaries remain the only mutation path. External integrations produce staged proposals or commands; they do not write operational tables or TigerBeetle directly.
4. UI/UX post-MVP planning is represented through a spreadsheet-first tiled workspace and transposed record/header views. Tiles are governed lenses, not alternate business-state machines.
5. `command_event_seq` is part of command-derived outbox event identity and is unique per `(tenant_id, command_id, command_event_seq)`.
6. Validation rejects duplicate YAML keys, duplicate active ADR/gate IDs, stale active P0 gate versions, missing referenced fixtures, missing SQL invariants, and unresolved `ci://` evidence from gate cards.

### Phase 0 boundary

No external connector runtime, broker, CDC, TigerBeetle, pgvector, DuckDB, marketplace SDK, or full tiled workspace is admitted to the Phase 0 vertical-slice hot path. Phase 0 may add schema fields, fixture contracts, command/outbox metadata, and minimal selected-record detail/transpose UI scaffolding only where they reduce future migration risk.


## v0.14.2 Review Closure: Spec Sync, Integration Handoff, and Credential Depth

v0.14.2 closes the v0.14.1 review without widening MVP scope.

Normative additions:

```text
- The v0.14.2 spec is archived; v0.15.0 is now the only active spec in /spec.
- Integration staging cannot create a command proposal until scan, schema, size, content type, service-account scope, credential state, classification, and mapping checks pass.
- Credential refs require KMS/envelope-backed secret storage, rotation overlap, revocation propagation, audited access, and CI secret scanning.
- P1-UX-001 is post-vertical-slice; Phase 0 only preserves command-safe UI metadata hooks.
- Integration-derived data entering pgvector, DuckDB, TigerBeetle projections, or UI tiles remains governed by source version, classification, lineage, and revalidation.
```
...
tests/fixtures/integration/pilot-v1-small-import-negative.json
```


## v0.15.0 Final Promotion Closure: Version, Handoff, Credential, Observability, and Fixture Cleanup

v0.15.0 closes the v0.14.2 final-promotion review without widening MVP scope.

Normative additions:

```text
- Active README, pack-index, spec, SLOs, invariants, manifest, and validation now target v0.15.0.
- Command lifecycle contains a single concise external staging -> validation gates -> command proposal -> command-handler sequence.
- Credential management defines a tenant-scoped DEK + KMS master-key envelope-encryption profile.
- Revoked credentials must be observed by queued and in-flight workers before external calls and delivery-attempt writes.
- Integration observability includes staging, scan, schema validation, dead-letter, and credential-revocation telemetry.
- Negative integration fixtures include revoked credential + schema mismatch and must never create command proposals.
- P1-UX-001 remains post-vertical-slice.
```
...
docs/review/critical-review-v0.15.0.md
docs/dev/command-lifecycle.md
docs/security/integration-credential-management.md
docs/observability/phase0-observability.md
tests/fixtures/integration/pilot-v1-revoked-credential-schema-mismatch.json
```


---

## 28. v0.15.0 AI Coding-Agent Implementation Roadmap

The detailed execution roadmap is normative for agent-assisted implementation, but it does not introduce new runtime product scope. The canonical documents are:

| Concern | Canonical document | Gate/evidence |
|---|---|---|
| Agent operating rules | `AGENTS.md` | `EXEC-001`, `EXEC-003` |
| Roadmap and dependency DAG | `docs/implementation/ai-coding-agent-roadmap.md` | `P0-EXEC-001` |
| Phase 0 work orders | `docs/implementation/phase0-agent-work-orders.md` | `EXEC-002`, `EXEC-004` |
| Agent role model and handoffs | `docs/implementation/agent-operating-model.md` | `EXEC-005` |
| PR validation playbook | `docs/implementation/agent-pr-validation-playbook.md` | `EXEC-003` |
| Agent QA matrix | `docs/qa/agent-implementation-validation-plan.md` | `BENCH-EXEC-001` |

### 28.1 Agent execution rules

```text
1. One work order per PR unless an owner explicitly approves otherwise.
2. Agents must run `scripts/validate-pack.sh` before handoff.
3. Agents may not implement post-MVP planes in the Phase 0 edit path.
4. Agents may not bypass command handlers, outbox envelopes, RetrievalRevalidator, or invariant CI.
5. Agents must include tests/evidence named in the relevant gate.
6. Agents must stop on ambiguity rather than inventing unstated semantics.
```

### 28.2 First implementation target

The first implementation target remains the vertical slice:

```text
safe cell edit -> command claim -> PostgreSQL transaction -> audit/domain/outbox -> polling SSE -> command status recovery
```

The agent roadmap starts with repository bootstrap and validation, then implements P0-CMD-001 and P0-LIVE-001 before any wider grid, integration, AI, analytics, or tiled workspace work.
