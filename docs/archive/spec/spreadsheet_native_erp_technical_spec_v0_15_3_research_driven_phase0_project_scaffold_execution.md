...
## 2. v0.15.3 Review-Closure Additions

| Area | v0.15.1 residual polish item | v0.15.3 refinement |
|---|---|---|
| Cognitive load | New agents had to traverse many normative files before editing. | Add `docs/snapshot-v0.15.3.md` and `SNAP-001`; make the snapshot the first required read path. |
...
| Simulation evidence | The simulation test plan existed but did not attach a concrete run artifact. | Add `docs/qa/agent-simulation-run-v0.15.3.md`, `EXEC-009`, and visible SLO targets. |
...
|---|---|
| First-read architecture snapshot | `docs/snapshot-v0.15.3.md` |
...
## 14. v0.15.3 Agent Handoff and Simulation Evidence

v0.15.3 adds three low-scope delivery accelerators:
...
## 15. v0.15.3 Visible Execution SLOs
...
## v0.15.3 Project Scaffold and Repository Layout

v0.15.3 turns the documentation pack into an implementation-shaped monorepo skeleton. This does not admit new product runtime scope. It creates the directories, package manifests, agent instructions, and TypeScript stubs needed for Phase 0 agents to begin implementation without inventing repository topology.

### v0.15.3 additions

| Area | Change | Scope control |
|---|---|---|
| Repo shape | Add `apps/` and `packages/` workspaces, root `package.json`, `pnpm-workspace.yaml`, TypeScript configs, and GitHub CI stubs. | Structure only; no production runtime behavior is implied. |
| App stubs | Add `apps/web`, `apps/api`, and `apps/worker-outbox` stubs. | MVP edit path remains command-first and polling-first. |
| Package stubs | Add `packages/contracts`, `packages/domain`, `packages/db`, `packages/observability`, `packages/testkit`, `packages/ui`, and `packages/config`. | Post-MVP planes stay feature-flagged off. |
| Agent instructions | Add subsystem `AGENTS.md` files plus `.github/copilot-instructions.md` and `.cursor/rules/phase0-boundaries.mdc`. | Agent instructions repeat stop conditions; they do not override root `AGENTS.md`. |
| Git hygiene | Add `.gitignore`, `.env.example`, and archived historical root reports/changelogs. | Secrets, generated artifacts, local databases, and build outputs are ignored. |
| Scaffold validation | Add `STRUCT-001`, `STRUCT-002`, and `STRUCT-003` to require the project skeleton, workspace agent instructions, and runtime-scope guardrails. | Scaffold checks are process guardrails, not product features. |

### Non-negotiable scaffold rule

```text
The repo structure may guide implementation, but it does not change system authority.
All mutations still go through command handlers.
Outbox remains the only outbound event authority.
Post-MVP planes stay out of the ordinary Phase 0 edit path.
```


## 16. Final v0.15.3 Recommendation

Proceed with Phase 0 implementation using the v0.15.3 agent work-order catalog. The first sprint should implement repository bootstrap, command identity, outbox polling scaffolding, invariant CI, and a minimal command-safe spreadsheet edit path.
