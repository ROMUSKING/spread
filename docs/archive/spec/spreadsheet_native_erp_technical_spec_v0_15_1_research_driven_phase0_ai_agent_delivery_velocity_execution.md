# Spreadsheet-Native ERP Technical Specification v0.15.1

**Date:** 2026-06-26  
**Status:** Phase 0 implementation-readiness baseline with AI coding-agent execution roadmap and delivery-velocity controls  
**Supersedes:** v0.15.0 AI coding-agent implementation roadmap baseline  
**Version note:** This is **v0.15.1**, not v1.0. Version 1.0 remains reserved for a release-candidate baseline after Phase 0 evidence exists.  
**Audience:** Phase 0 engineering, AI coding agents, reviewers, QA, SRE, security, compliance, product, and domain owners.

## 1. Executive Summary

v0.15.1 converts the v0.15.0 agent roadmap into a more executable, lower-cognitive-load implementation baseline. It adds a one-page architecture snapshot, code skeletons for agent PRs, a validation-waiver policy for non-release-blocking checks, a tech-stack and repository-shape snapshot, explicit UI mutation guardrails, and a post-MVP plane extraction document.
...
## 2. v0.15.1 Review-Closure Additions

| Area | v0.15.0 residual risk | v0.15.1 refinement |
|---|---|---|
| Cognitive load | New agents had to traverse many normative files before editing. | Add `docs/snapshot-v0.15.1.md` and `SNAP-001`; make the snapshot the first required read path. |
...
|---|---|
| First-read architecture snapshot | `docs/snapshot-v0.15.1.md` |
...
## 14. Final v0.15.1 Recommendation

Proceed with Phase 0 implementation using the v0.15.1 agent work-order catalog. The first sprint should implement repository bootstrap, command identity, outbox polling scaffolding, invariant CI, and a minimal command-safe spreadsheet edit path.
