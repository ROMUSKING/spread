...
## v0.14.2 Implementation Closure: Spec Sync, Integration Handoff, and Secret Boundary

v0.14.2 closes the v0.14.1 review by making the active specification match the review-closure artifacts and by tightening implementation guidance for external integrations. It does not widen Phase 0 or MVP runtime scope.

### Normative additions

1. The active normative specification is this v0.14.2 file. Older v0.14 and v0.14.1 specifications remain historical references only.
2. Inbound integration staging may create a command proposal only after authentication, per-connection rate limiting, byte/content-type validation, malware scan or quarantine handling, schema validation, classification/redaction, service-account scope checks, and mapping checks pass.
3. Integration credential material is managed through secret references, KMS-backed envelope encryption, rotation overlap windows, revocation propagation, audit logging, and CI secret scanning.
4. `docs/dev/command-lifecycle.md` now includes the external staging → command proposal → command handler sequence. The command handler remains the only business mutation path.
5. `P1-UX-001` is explicitly post-vertical-slice. Tiled workspace work must not distract from P0-CMD-001 and P0-LIVE-001.
6. Integration-derived data entering pgvector, DuckDB, AI, or mixed-plane retrieval must pass the integration security boundary and RetrievalRevalidator before becoming user-visible.

### Integration security posture summary

```text
Inbound external payload
  -> authenticate
  -> per-connection rate limit
  -> content-type + byte limit
  -> payload hash
  -> malware scan / quarantine
  -> schema/schema-evolution validation
  -> classification + redaction
  -> service-account scope check
  -> external mapping lookup
  -> command proposal
  -> user/system approval where required
  -> command handler
```

Any failed stage leaves the payload in rejected/quarantined/dead-letter state and must not produce a command proposal.

### Phase 0 boundary

Phase 0 may prepare schema fields, outbox envelope metadata, fixtures, validation checks, and minimal command-proposal scaffolding. Phase 0 must not introduce marketplace connectors, general inbound sync, third-party webhook delivery, connector runtime dependencies, or tiled workspace implementation into the vertical-slice hot path.
